...
type LineKind = "log" | "warn" | "error" | "info";
...
function isValidSnippetIndex(index: number): boolean {
...
function initUsersSection(): void {
  const statusElement = query<HTMLElement>("#status");
  const usersContainer = query<HTMLElement>("#usersContainer");
  const reloadButton = query<HTMLButtonElement>("#reloadButton");
...
function initStudioSection(): void {
  const exercisesContainer = query<HTMLElement>("#exercisesContainer");
  ...
  exercisesContainer.addEventListener("click", (event) => {
    ...
    const openButton = target.closest<HTMLButtonElement>(".open-terminal-btn");
    if (!openButton) {
      return;
    }
...
function initTerminalSection(): void {
  const tsEditor = query<HTMLTextAreaElement>("#tsEditor");
  const lineNumbers = query<HTMLElement>("#lineNumbers");
  const runCodeButton = query<HTMLButtonElement>("#runCodeButton");
  const clearConsoleButton = query<HTMLButtonElement>("#clearConsoleButton");
  const terminalOutput = query<HTMLElement>("#terminalOutput");
  ...
  const TAB_SPACES = "  ";
  let compilerLoadAttempt: Promise<TypeScriptCompiler | null> | null = null;
  ...
  function appendTerminalLine(message: string, kind: LineKind = "log"): void {
    const className =
      kind === "error"
        ? "line-error"
        : kind === "warn"
  ...
  function insertTabAtCursor(): void {
  ...
  function loadSnippet(index: number): void {
  ...
  async function ensureTypeScriptCompiler(): Promise<TypeScriptCompiler | null> {
    if (window.ts) {
      return window.ts;
    }
  ...
  async function executeTypeScript(): Promise<void> {
    ...
    if (!compiler) {
      appendTerminalLine(
        "No se pudo cargar TypeScript. Ejecuta desde la raiz del proyecto con: npm run serve:web",
        "error",
      );
      return;
    }
